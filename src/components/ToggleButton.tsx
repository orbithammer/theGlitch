...
const ToggleButtonInner = styled.div<ToggleButtonProps>`
  position: absolute;
  top: 0.2rem;
  left: ${({ $isDarkMode }) => ($isDarkMode ? '1.5rem' : '0.2rem')};
  width: 1.6rem;
  height: 1.6rem;
...
const ToggleButton = () => {
  ...
  return (
    <ToggleButtonContainer $isDarkMode={internalIsDarkMode} onClick={toggleTheme}>
      <ToggleButtonInner $isDarkMode={internalIsDarkMode} />
